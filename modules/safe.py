...
class RestrictedUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        ...
        raise ImportError(f"global '{module}/{name}' is forbidden")
...
def check_zip_filenames(filename, names):
    for name in names:
        ...
        raise ValueError(f"bad file inside {filename}: {name}")
    ...
    try:
        ...
        with zipfile.ZipFile(filename) as z:
            ...
            if not data_pkl_filenames:
                raise FileNotFoundError(f"data.pkl not found in {filename}")
            if len(data_pkl_filenames) > 1:
                raise ValueError(f"Multiple data.pkl found in {filename}")
