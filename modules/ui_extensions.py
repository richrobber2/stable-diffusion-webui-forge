...
def install_extension_from_url(dirname, url, branch_name=None):
    ...
    try:
        return _extracted_from_install_extension_from_url_24(
            tmpdir, branch_name, url, target_dir
        )
    finally:
        shutil.rmtree(tmpdir, True)


# TODO Rename this here and in `install_extension_from_url`
def _extracted_from_install_extension_from_url_24(tmpdir, branch_name, url, target_dir):
    shutil.rmtree(tmpdir, True)
    if not branch_name:
...
def refresh_available_extensions_from_data(selected_tags, showing_type, filtering_type, sort_column, filter_text=""):
    ...
    for ext in sorted(extlist, key=sort_function, reverse=sort_reverse):
        ...
        if filter_text and filter_text.strip() and (filter_text.lower() not in html.escape(name).lower() and filter_text.lower() not in html.escape(description).lower()):
            hidden += 1
            continue
    ...
    with gr.Blocks(analytics_enabled=False) as ui:
        with gr.Tabs(elem_id="tabs_extensions"):
            with gr.TabItem("Installed", id="installed"):
                ...
                extensions_table = _extracted_from_create_ui_12(ui)
            with gr.TabItem("Available", id="available"):
                _extracted_from_create_ui_56(modules, extensions_table)
            with gr.TabItem("Install from URL", id="install_from_url"):
                _extracted_from_create_ui_120(modules, extensions_table)
            with gr.TabItem("Backup/Restore"):
                _extracted_from_create_ui_133(modules, ui)
    return ui


# TODO Rename this here and in `create_ui`
def _extracted_from_create_ui_133(modules, ui):
...
# TODO Rename this here and in `create_ui`
def _extracted_from_create_ui_120(modules, extensions_table):
...
# TODO Rename this here and in `create_ui`
def _extracted_from_create_ui_56(modules, extensions_table):
    with gr.Row():
...
# TODO Rename this here and in `create_ui`
def _extracted_from_create_ui_12(ui):
    ...
    with gr.Row(elem_classes="progress-container"):
        result = gr.HTML('Loading...', elem_id="extensions_installed_html")

    ui.load(fn=extension_table, inputs=[], outputs=[result], show_progress=False)
    refresh.click(
        fn=extension_table, inputs=[], outputs=[result], show_progress=False
    )
